...
public class UserMenu {
    ...
    public static void handleUserActions(Scanner scanner, UserController userController, User currentUser, GameRepository gameRepository, ChatRepository chatRepository) {
        while (true) {
            ...
            System.out.println("9. View Game Details");
            ...
            switch (choice) {
                ...
                case 9 -> viewGameDetails(scanner, gameRepository, currentUser);
                case 10 -> {
    ...
    private static void viewGameDetails(Scanner scanner, GameRepository gameRepository, User currentUser) {
        System.out.println("\n=== Available Games ===");
        List<Game> games = gameRepository.getAllGames();
        for (Game game : games) {
            System.out.printf("ID: %d | Name: %s | Price: $%.2f | Age Restriction: %d+\n",
        ...
        System.out.print("\nEnter Game ID to view details (or 0 to go back): ");
        int gameId = scanner.nextInt();
        scanner.nextLine();
        ...
        if (gameId != 0) {
            Game selectedGame = gameRepository.getGameById(gameId);
            if (selectedGame != null) {
                handleGameDetails(scanner, gameRepository, selectedGame, currentUser);
            } else {
                System.out.println("Game not found.");
            }
        }
